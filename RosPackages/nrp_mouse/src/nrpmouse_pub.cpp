...
double allForeKneeServoAngle(double ft1,double ft2);
double allForeKneeServoAngle(double ft1,double ft2);
double allHindKneeServoAngle(double ht1,double ht2);
double allHindKneeServoAngle(double ht1,double ht2);
double allForeHipServoAngle(double ft1);
double allForeHipServoAngle(double ft1);
double allHindHipServoAngle(double ht1);
double allHindHipServoAngle(double ht1);
double kneeservoangle(double l);
float degToRad(double l);
//double M_PI=3.1428;
...
double streamfreqB=40; 	//how many set of values are published to the teensy per second. (maximum of 500 values per second)
...
int main(int argc, char **argv)
{
    // ROS objects
  	ros::init(argc, argv, "nrpmouse_pub");
        ...
        std_msgs::Float64 LElbow;
        std_msgs::Float64 LShoulder;
        std_msgs::Float64 RElbow;
        std_msgs::Float64 RShoulder;
        std_msgs::Float64 LHip;
        std_msgs::Float64 LKnee;
        std_msgs::Float64 RHip;
        std_msgs::Float64 RKnee;
        std_msgs::Float64 LBodyH;
        std_msgs::Float64 LTail;
	...
	ros::NodeHandle n;
        ...
        ros::Publisher LElbow_pub = n.advertise<std_msgs::Float64>("/Nermo/joint_L_Elbow/cmd_pos", 512);
        ros::Publisher LShoulder_pub = n.advertise<std_msgs::Float64>("/Nermo/joint_L_Shoulder/cmd_pos", 512);
        ros::Publisher RElbow_pub = n.advertise<std_msgs::Float64>("/Nermo/joint_R_Elbow/cmd_pos", 512);
        ros::Publisher RShoulder_pub = n.advertise<std_msgs::Float64>("/Nermo/joint_R_Shoulder/cmd_pos", 512);
        ros::Publisher LHip_pub = n.advertise<std_msgs::Float64>("/Nermo/joint_L_Hip/cmd_pos", 512);
        ros::Publisher LKnee_pub = n.advertise<std_msgs::Float64>("/Nermo/joint_L_Knee/cmd_pos", 512);
        ros::Publisher RHip_pub = n.advertise<std_msgs::Float64>("/Nermo/joint_R_Hip/cmd_pos", 512);
        ros::Publisher RKnee_pub = n.advertise<std_msgs::Float64>("/Nermo/joint_R_Knee/cmd_pos", 512);
        ros::Publisher LBodyH_pub = n.advertise<std_msgs::Float64>("/Nermo/joint_Body_H/cmd_pos", 512);
        ros::Publisher LTail_pub = n.advertise<std_msgs::Float64>("/Nermo/joint_Tail/cmd_pos", 512);


        ros::Rate loop_rate(streamfreqB); //run at Hz, not sleep time
	...
	{	
		...
		{	highfiveleft();
	  	...
	  	{	
                        ...
                        //msgarr.data[0]=(NRPMouseservoarray[p][0]);
                        LShoulder.data=(degToRad(NRPMouseservoarray[p][1]))-1.8;
                        LElbow.data=(degToRad(NRPMouseservoarray[p][2])/4)-1;
                        RShoulder.data=(degToRad(NRPMouseservoarray[p][3]))-1.8;
                        RElbow.data=(degToRad(NRPMouseservoarray[p][4])/4)-1;
                        LHip.data=(degToRad(NRPMouseservoarray[p][5]))-0.15;
                        LKnee.data=(-degToRad(NRPMouseservoarray[p][6])/2)+0.85;
                        RHip.data=(degToRad(NRPMouseservoarray[p][7]))-0.15;
                        RKnee.data=(-degToRad(NRPMouseservoarray[p][8])/2)+0.85;
                        LBodyH.data=degToRad(NRPMouseservoarray[p][9]);
                        LTail.data=degToRad(NRPMouseservoarray[p][10]);
			...
			//std::cout<<NRPMouseservoarray[p][9]<<std::endl; 
                        ...
                        LBodyH_pub.publish(LBodyH);
                        LTail_pub.publish(LTail);
			ros::spinOnce();
			loop_rate.sleep();
	  	}
	}	
...
void ikforeleft(double fx,double fy)
{
    ...
    ft1=ft1*180/M_PI;
    ft2=ft2*180/M_PI;
    fla1=allForeHipServoAngle(ft1);
    fla2=allForeKneeServoAngle(ft1,ft2);
}
...
void ikforeright(double fx,double fy)
{
    ...
    ft1=ft1*180/M_PI;
    ft2=ft2*180/M_PI;
    fra1=allForeHipServoAngle(ft1);
    fra2=allForeKneeServoAngle(ft1,ft2);
}
...
void ikhindleft(double hx,double hy)
{
    ...
    ht1=ht1*180/M_PI;
    ht2=ht2*180/M_PI;
    hla1=allHindHipServoAngle(ht1);
    hla2=allHindKneeServoAngle(ht1,ht2);
}
void ikhindright(double hx,double hy)
{
    ...
    ht1=ht1*180/M_PI;
    ht2=ht2*180/M_PI;
    hra1=allHindHipServoAngle(ht1);
    hra2=allHindKneeServoAngle(ht1,ht2);
}
...
//    double centerdist;
//    double lhip;
//    double flsi=48.332; 											//flsi=41.198+(fahip*M_PI/180*frhip); //initial calculated length
...
//    double centerdist;
//    double lhip;
//    double hlsi=47.477;      										//hlsi=37.687+(hahip*M_PI/180*hrhip); //initial total length
